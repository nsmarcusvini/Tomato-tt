...
insert into dados_historicos (dtUmidade,medidasUmidade,dtTemperatura,medidaTemperatura,statusSensor) values
...
alter table usuario add column fkCadastro_usuario int;
alter table usuario add foreign key (fkCadastro_usuario) references cadastro (idCliente);

alter table usuario add column fkFazenda_usuario int;
alter table usuario add foreign key (fkFazenda_usuario) references fazenda (idFazenda);

alter table sensor add column fkFazenda_sensor int;
alter table sensor add foreign key (fkFazenda_sensor)references fazenda (idFazenda);

alter table dados_historicos add column fkSensor_dados int;
alter table dados_historicos add foreign key (fkSensor_dados) references sensor (idSensor);


select * from usuario join cadastro on fkCadastro_usuario = idUsuario
					  join fazenda on fkFazenda_usuario = idFazenda;
